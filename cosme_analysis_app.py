...
if submitted and url_input:
    ...
    if df.empty:
        ...
        fig3,ax3=plt.subplots(); df['sentiment'].hist(bins=20,ax=ax3); st.pyplot(fig3)
        # セグメント
        st.subheader("年代×クラスタ 分布")
        tfidf=TfidfVectorizer(max_features=30,stop_words="japanese"); X=tfidf.fit_transform(df['本文'])
        km=KMeans(n_clusters=3,random_state=42,n_init=10).fit(X); df['クラスタ']=km.labels_
        seg=pd.crosstab(df['年代'],df['クラスタ']); st.dataframe(seg)
        # ダウンロード
        csv=df.to_csv(index=False).encode('utf-8-sig'); st.download_button("CSVダウンロード",data=csv,file_name="cosme_reviews.csv",mime="text/csv")
